...
if ('' === $route) {
    $route = (string) ($_SERVER['BABELCHROME_MODULE_ROUTE'] ?? 'json');
}
...
$response = (new JsonViewerModule())->handle(new ModuleRequest(
    new ModuleManifest(
        is_string($manifestData['id'] ?? null) ? $manifestData['id'] : 'babelforge.json-viewer',
        is_string($manifestData['name'] ?? null) ? $manifestData['name'] : 'JSON Viewer',
